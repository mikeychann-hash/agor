...
Quick intro: I’m Max (`@mistercrunch` on GitHub). A while back I created [Apache Airflow](https://github.com/apache/airflow) and [Apache Superset](https://github.com/apache/superset). Everyone in data knows Airflow. And everyone—period!—should know Superset. It’s an AMAZING open source project quietly taking over BI. Tableau/Looker-killer levels. It’s ready to take over the world → check it out.
...
From the moment this got good, it was obvious SDLC would never be the same ([why: adopting Claude Code and the software economics singularity](https://preset.io/blog/adopting-claude-code-riding-the-software-economics-singularity/)). I told my team to drop everything, put every engineer/PM/designer on Claude Max Pro, and started doing frequent brown-bags to figure out how to channel this insanity. I wrote little tools (like [Claudette](https://preset.io/blog/introducing-claudette-cli-supercharge-your-apache-superset-development/)) to sand down friction and let my own workflow reshape around agents.
...
- Board, not list: Make the 2D canvas the primary UX. Spatial memory, presence, and orchestration beat vertical feeds. You remember where things live; teammates see motion in real time.
- Worktrees over branches: Isolate by feature, not by session. Forks copy conversation context only; they share the same filesystem. No “git branch explosion” per chat.
- SDK-first integrations: Orchestrate Claude Code, Codex, Gemini via SDKs. Treat CLIs as optional adapters, not the core.
- Zero‑config MCP: Session‑scoped token auto‑injected into `mcpServers.agor` on the same daemon. No extra process, no manual setup.
- Environments as first‑class: Per‑worktree start/stop with unique ports from simple templates. Share a running env across sessions; avoid port roulette.
- Fast UI primitives: Ant Design + React Flow. Ship the model before the paint job; save custom components for when they’re undeniably needed.
- Zones as automation: Drag‑to‑trigger workflows powered by Handlebars templates with board/worktree/session/env context. No more copy‑pasting issue/PR links into prompts.
- Async by default: Spawns don’t block parents. Reports are explicit (files/messages). Add callbacks later only if they clearly reduce friction.
- Real‑time everywhere: Broadcast Feathers service events over WebSocket; optimistic UI; minimal polling.
- Collage > reinvention: Use simple‑git, Drizzle, Socket.io, and friends. Spend calories on orchestration, not on rebuilding solved layers.
- Bring your own editor: Don’t ship a heavy embedded editor. Connect VSCode/Cursor/etc. to running worktrees; meet people where they already work.
- Scope guardrails: Fewer agents at first, deeper capability later. No “smart review” L2 until base flows proved stable and valuable.
...
A multiplayer, spatial layer that coordinates agentic coding across tools and teams: real-time boards, session trees, zone-triggered workflows, isolated git worktrees with one-click environments, and the right glue to orchestrate Claude Code, Codex, Gemini, and friends. It’s the place where the work—and the context that created it—actually lives. The code is open at [github.com/mistercrunch/agor](https://github.com/mistercrunch/agor).
